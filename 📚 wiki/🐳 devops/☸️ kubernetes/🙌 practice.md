...
- `NodePort` - можно открыть порт в интернет (но неудобно, потому что не классические порты)
- `LoadBalancer` - используется только в облачных средах
- `ExternalName` - обращение к сервису по конкретному имени, которое можно задать
- `ExternalIPs` - как NodePort, только можно отдать прямо IP адреса
